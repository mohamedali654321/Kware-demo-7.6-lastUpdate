...
export class SearchLabelComponent implements OnInit {
  ...
  normalizeFilterValue(value: string) {
    ...
    if(this.filterName === 'author'  || this.filterName === 'advisor' || this.filterName === 'person'){
      this.value=(this.localeService.getStringByLocale(this.value?.split(",")[0]) +',' +this.localeService.getStringByLocale(this.value?.split(",")[1]))
    }
